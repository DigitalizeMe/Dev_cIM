    ...
    for s in g.subjects(RDF.type, OULD.UpdatableEntity):
        if not list(g.objects(s, OULD.consistsOf)):  # No subcomponents
            ...
            for phase in phases:
                ...
                selected = input(f"Which instants for {s} in Phase {phase}? (e.g., '1, 2' or 'abort') ").strip()
                ...
                phase_uri = phase_uris[phase]
                g.add((s, OCCP.hasPhase, phase_uri))  # Link component to phase

                for num in selected_nums:
                    instant_label = next(label for n, label in instant_options if n == num)
                    instant_uri = EX[f"instant_{instant_label}"]
                    ...
                    while True:
                        ...
                        if timestamp.lower() == "abort":
                            ...
                            print("Process aborted. Partial ABox saved to:", output_file)
                            return
                        if re.match(r"^\d{4}-\d{2}-\d{2}$", timestamp):  # Basic xsd:date check
                            break
                        print("Invalid format. Please use 'YYYY-MM-DD'.")

                    # Ask if estimated or actual
                    while True:
                        est = input("Is this timestamp estimated (J/N)? ").strip().upper()
                        if est.lower() == "abort":
                            g.serialize(destination=output_file, format="turtle")
                            print("Process aborted. Partial ABox saved to:", output_file)
                            return
                        if est in ["J", "N"]:
                            break
                        print("Please enter 'J' or 'N'.")
                    
                    time_prop = OCCP.hasEstimatedTime if est == "J" else OCCP.hasActualTime
